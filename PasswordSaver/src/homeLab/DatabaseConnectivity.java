    ...
    public void Insert(DatabaseEntry entry) {
        try {
            ...
            pStmt = c.prepareStatement(
                    "INSERT INTO PASSWORDS (ID,PASSWORD,USERNAME,EMAIL,WEBSITE) " +
                    "VALUES ("+currentID+", ?, '"+entry.getUsername()+"', '"+entry.getEmail()+"', '"+entry.getWebsite()+"' );"
            );
            pStmt.setBytes(1,entry.getPassword());
            pStmt.executeUpdate();
            pStmt.close();
            c.commit();
